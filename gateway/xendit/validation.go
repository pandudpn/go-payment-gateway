...
func validationCreateVirtualAccount(va *CreateVirtualAccountParam) error {
	...
	if !checkBankCode(va.BankCode) {
		return fmt.Errorf("%s. possible values are BankBCA, BankBNI, BankBRI, BankMandiri, BankBSI, BankBJB, BankCIMB, BankDBS, BankPermata, BankSahabatSampoerna", pg.ErrInvalidParameter)
	}
	...
	if va.IsClosed {
		...
		switch va.BankCode {
		case BankPermata:
			if va.ExpectedAmount > 9999999999 {
		...
		default:
			if va.ExpectedAmount > 50000000000 {
